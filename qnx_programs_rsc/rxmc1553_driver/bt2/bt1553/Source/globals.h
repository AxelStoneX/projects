...
EXTERN BT_INT  supMF_OFlow[MAX_BTA];   /* Ignore BC MF overflow if set.        */
EXTERN BT_INT  _HW_1Function[MAX_BTA]; /* Hardware supports single function.   */
EXTERN BT_INT  _HW_FPGARev[MAX_BTA];   /* PCI/ISA LPU(FPGA) Version.           */
EXTERN BT_INT  _HW_WCSRev[MAX_BTA];    /* Writable Control Store Version.      */
EXTERN BT_UINT _HW_PROMRev[MAX_BTA];   /* PROM Version                         */
...
EXTERN BT_INT CurrentMemKB[MAX_BTA];     /* Card memory in KB (1024 bytes)     */
...
EXTERN BT_U8BIT MultipleBMTrigEnable[MAX_BTA]; /* Repetitive triggers enabled. */
EXTERN BT_U8BIT BroadcastIntEnable[MAX_BTA][32]; /* Broadcast ints enabled.    */
...
EXTERN BT_INT board_has_serial_number[MAX_BTA];
...
EXTERN BT_U16BIT heartbeat[MAX_BTA];
EXTERN API_CHANNEL_STATUS channel_status[MAX_BTA];
EXTERN BT_INT irig_bias[MAX_BTA];
EXTERN BT_INT channel_is_shared[MAX_BTA];
EXTERN BT_UINT rxmc_output_config[MAX_BTA];
...
EXTERN BT_U8BIT  bc_inited[MAX_BTA];       // Non-zero indicates BC initialized
EXTERN BT_U8BIT  bc_running[MAX_BTA];      // Non-zero indicates BC is running
EXTERN BT_U16BIT frametime[MAX_BTA];       // Minor frame time, 25us LSB.
EXTERN BT_U16BIT wResponseReg4[MAX_BTA];   // response_reg[0x04] contents.
...
#define BTMEM_BM_BEGIN     0x00000L    /* Beginning address of BM memory   */
EXTERN BT_U8BIT  bm_inited[MAX_BTA];   // BM has been initialized
EXTERN BT_U8BIT  bm_running[MAX_BTA];  // BM is running flag
...
//EXTERN API_BM_MBUF *lpAPI_BM_Buffers[MAX_BTA]; // API BM msg queue ptr.
...
/****************************************************************************
*  Local copy of registers -- the main use is to provide the ability to
*  set a single bit in a hardware register without knowing what else is
*  there (since we can't read all of the hw registers).  See the
*  routines "api_writehwreg_or" and "api_writehwreg_and".
****************************************************************************/
...
EXTERN BT_U8BIT rt_inited[MAX_BTA];          // RT initialized flag.
EXTERN BT_U8BIT rt_running[MAX_BTA];         // RT running flag.
EXTERN BT_U8BIT rt_bcst_enabled[MAX_BTA];    // RT address 31 is broadcast flag.
EXTERN BT_U8BIT rt_sa31_mode_code[MAX_BTA];  // RT subaddress 31 is mode code.
...
EXTERN BT_U32BIT btmem_rt_begin    [MAX_BTA];  // RT seg base: 0 
...
EXTERN BT_U32BIT btmem_pci1553_rt_mbuf[MAX_BTA];/* Top of memory, PCI-1553.   */
...
#define BTMEM_RT_TOP_BRO   (2*0xE160L) /* Top addr(bro) avail in RT memory   */
#define BTMEM_RT_TOP_NOBRO (2*0xF120L) /* Top addr(nobro) avail in RT memory */
#define BTMEM_RT_BOT_BEGIN (2*0x0000L) /* Bot/first avail addr in RT memory  */
