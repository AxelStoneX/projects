 ...
 void* polling_timer_func(void* val) 
 {
    ...
    while(1)
    {
       ...
       for(cardnum=0; cardnum < MAX_BTA; cardnum++)
       {
          ...
          {
             if((temp = vbtGetRegister(cardnum, HWREG_HEART_BEAT)) == heartbeat[cardnum])
...
int vbtInterruptSetup(unsigned int cardnum, int hw_int_enable, int api_dev)
{
   ...
   *******************************************************************/
   ...
   if(hw_int_enable > 0 &&  hw_int_enable < 4)
   {
      ...
      vbtSetRegister(cardnum, HWREG_WRITE_INTERRUPT, 0);  // clear the interrupt write register
...
void intProc(int int_data)
{
   ...
   if ((id0[device] = InterruptAttach(hwevent[device].IRQnum, (void*) &irq_handler, &hwevent[device], sizeof(HWEVENT), _NTO_INTR_FLAGS_TRK_MSK)) == -1)
   {
      id0[device] = -1;
   }
   else
