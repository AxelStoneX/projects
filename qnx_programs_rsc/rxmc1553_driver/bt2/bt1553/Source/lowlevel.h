 ...
 *============================================================================*
 *
 *      COPYRIGHT (C) 1997-2010 BY GE - INTELLIGENT PLATFORMS, INC.  ALL
 *      RIGHTS RESERVED.
 *
...
// Define the max number of logical devices (cardnum's) we support
#define MAX_CARDNUM   16            /* V4.06 */
...
//
//
// The following sections support the CEI-x20 API
//
//
//---------------------------------------------------------------------------*
...
{
   CEI_UINT32 num_mem_rgns_mapped;   // the number of memory regions 
                                     // that have been mapped
   CEI_CHAR *rgn_host_addr[MAX_MEMORY];  // base address of each mapped memory 
                                     // region in host space
   CEI_CHAR *rgn_phys_addr[MAX_MEMORY];  // physical base address of each 
                                     // memory region
...
/*===========================================================================*
 * ENTRY POINT: vbtMapBoardAddressesX20
 *===========================================================================*
 *
 * FUNCTION:    Deprecated routine - use vbtMapBoardAddressesExtX20 instead.
 *
 * RETURN VAL:  see vbtMapBoardAddressesExtX20
 *
 * DESCRIPTION: This routine has been deprecated and will be removed in a 
 *              future distribution.  Use vbtMapBoardAddressesExtX20 instead.
 *
 *===========================================================================*/
CEI_INT32 vbtMapBoardAddressesX20
(
   CEI_UINT32 board,               // (i) board index (0 - 9)
   PCEIX20_MEMORY_MAP_T map        // (o) receives mapping info (if successful)
);   
...
/*===========================================================================*
 * ENTRY POINT: vbtHookInterruptX20
 *===========================================================================*
 *
...
/*===========================================================================*
 * ENTRY POINT: vbtUnhookInterruptX20
 *===========================================================================*
 *
 ...
 *
 *===========================================================================*/
CEI_INT32 vbtUnhookInterruptX20
(
   CEI_UINT32 board,               // (i) board index (0 - 9)
   PCEIX20_MEMORY_MAP_T map        // (i) mapping structure obtained by prior
                                   //     call to vbtMapBoardAddressesX20
...
/*===========================================================================*
 * ENTRY POINT: vbtMapSharedMemoryX20
 *===========================================================================*
 *
 ...
 *
 *===========================================================================*/
CEI_INT32 vbtMapSharedMemoryX20
(
...
/*===========================================================================*
 * ENTRY POINT: vbtSharedMutexInitX20
 *===========================================================================*
 *
 ...
 *
 *===========================================================================*/
CEI_INT32 vbtSharedMutexInitX20
(
   CEI_UCHAR mutex_id,             // (i) unique global lock id (0-255)
   CEI_UINT32 attach_to_existing,  // (i) true if the lock exists (see desc)
   CEI_SHARED_MUTEX *mutex_global, // (i/o) pointer to global mutex (see desc)
   CEI_SHARED_MUTEX **mutex_local  // (o) receives local mutex ptr (see desc)
);
...
/*===========================================================================*
 * ENTRY POINT: vbtSharedCondInitX20
 *===========================================================================*
 *
...
#if defined(_LINUX_X86_) || defined(VXW_VME_PPC) || defined(VXW_Q104_X86) || defined(VXW_PCI_PPC) || defined(VXW_PCI_THALES) \
                         || defined(VXW_PCI_X86) || defined(LYNXOS) || defined (VXW_DY4_PCI_PPC) || defined(VXW_VME_X86) || defined(VXW_PCI_MVPPC) || defined(CEIX20_TARGET_VXWORKS)
#ifndef _UNIX_
  #define _UNIX_
#ifndef HANDLE
#define HANDLE int
...
#endif //_UNIX_
#endif // (_LINUX_X86_) || defined(VXW_VME_PPC) ......
...
#ifdef _USE_BM_DMA
   unsigned *     vaddr;                      // 
   CEI_UINT64     laddr;                      // local DMA addres
#endif
   int            use_channel_map;            //
...
void  vbtFreeBoardAddresses(PDEVMAP_T pMap);  // Pointer to region mapping structure
...
void  vbtCloseInterrupt(UINT device, UINT cardnum);
...
int   vbtMapBoardAddress(unsigned    cardnum,     // Card Number to open
                         ...
                         int      addressing_mode); // for VME boars only.
#endif


void vbtFreeBoardAddress(UINT cardnum);
#ifdef _LVRT_
int   vbtMapVISAaddress(DWORD    cardnum,     // Card Number to open
                         DWORD    device,      // WinRT device to open (0 - 9)
                         char     **addr,      // computed host address
                         void    *MemorySize,  // byte size of region mapped
                         int      addressing_mode); // for VME boars only.
void vbtFreeVISAaddress(UINT cardnum);
#endif //_LVRT_


#if !defined(VXW_VME_68K) || !defined(LYNXOS_VME_PPC)
...
#ifdef WIN32
__int64 PerformanceCounter(void);
#endif
