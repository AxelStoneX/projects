...
// <<<---<<
typedef struct 
{ int  CHID;   // Channel ID to be used for a pulse receiption
  int  COID;   // Connection ID for a pulse receiption
  struct _pulse  PUL;
} PUL_CHAN;
volatile PUL_CHAN PUL_C[MAX_BTA];
// <<<---<<
...
void intProc(int int_data)
{
   int device,i;
   int irqN, iid;     // @@@QQ !!!
   int      RS, att_flags = 0;     // @@@QQ !!!
   pid_t    PID = 0;               // @@@QQ !!!
   uint32_t node = 0, att_ix = 0;  // @@@QQ !!!
  ...
  // <<<---<< -----------------------------------------------------
  PUL_C[device].CHID = ChannelCreate_r( _NTO_CHF_FIXED_PRIORITY );
  if(  PUL_C[device].CHID < 0 )
   { printf( "IWT: ChannelCreate failure %d !!!\n", - PUL_C[device].CHID ); return;  }

  // Node and pid == 0,
  PUL_C[device].COID = ConnectAttach_r( 
                           node, PID, PUL_C[device].CHID, att_ix, att_flags );
  if( PUL_C[device].COID < 0 )
   { printf( "IWT: ConnectAttach %d !!!\n", -PUL_C[device].COID ); return; }

  // SIGEV_PULSE_INIT( &(hwevent[device].event), PUL_C[device].COID, 33, 0, 1 );
  // <<<---<< -------------------------------------------------------

   SIGEV_INTR_INIT( &hwevent[device].event ); // @@@QQ -> PULSE_INIT is instead
   if ((id0[device] = 
   ...
   while(1)
   {
      InterruptWait(NULL,NULL); // @@@QQ -->  MsgReceivePulse() is instead
      // <<<---<< -------------------------------------------------------
      // RS = MsgReceivePulse( PUL_C[device].CHID,
      //          (void*)( &(PUL_C[device].PUL) ), sizeof(struct _pulse), NULL );
      // if( RS != 0 ) { printf("\nMsgReceivePulse error !!!\n");  break;  }
      // <<<---<< -------------------------------------------------------
