 ...
 *             Platforms avionics MIL-STD-1553 devices.
 *
 *
 * ENTRY POINTS:
...
/* $Revision:  1.15 Release $
  ...
  05/25/2006   replaced kill_proc with send_sig_info in uceipci_1553_isr. bch
  09/13/2006   added support for AMC-1553. bch
  02/19/2007   added support for EPMC-1553 and QPCX-1553. modified to support
                kernel changes in 2.6.19. bch
  05/30/2007   added intrpt_cntrl_1553. modified to support multiple processes. bch
  08/20/2007   modified to support kernel changes in 2.6.22. bch
  12/11/2007   added R15-EC. added AMC-1553 to intrpt_cntrl_1553. bch
  11/18/2008   added support for the R15-AMC. modified uceipci_1553_isr to
                support kernel changes in 2.6.26. bch
  02/27/2009   modified initialize_1553 by removing the probing and mapping of 
                BAR regions. modified uninitialize_1553 by removing the
                unmapping of BAR regions. added hardware interrupt status to 
                intrpt_cntrl_1553. modified debug output in uceipci_1553_isr. bch
  07/21/2009   added support for the RXMC-1553. bch	
  10/20/2009   added support for the RPCIe-1553. added attribute bus_type for sysfs.
                modified uceipci_1553_isr to support kernel changes in 2.6.31 - bch
  04/13/2011   added support for the R15-LPCIE and RXMC2-1553. bch
  08/12/2011   modified Endian check. bch
  10/14/2011   modified uceipci_1553_isr for get_pid_task and added lock/unlock. bch  
  08/07/2012   added support for RAR15-XMC-XT. bch
  10/09/2012   added support for RAR15-XMC and UCA32 firmware. bch
*/
...
#include <linux/sched.h>
 ...
 #else
  static irqreturn_t uceipci_1553_isr(int irq, void* data, struct pt_regs* regs);
 #endif
#endif  
...
static int initialize_1553(struct pci_dev* pdev) {
 ...
 if((uceipci_debug >= 1) && (pdev->device != 0x1003)) {
    unsigned short ver_wcs_major=0, ver_wcs_minor=0, ver_wcs_build=0, ver_lpu_major=0, ver_lpu_minor=0, ver_lpu_build=0;
    ...
    if(pData->uca32) {
      ...
      // WCS
      val16 = readw((unsigned int*)(pData->hwr_chan_addr[0]+0x30));
      if(uceipci_debug >= 2)
        printk(KERN_INFO "uceipci(%d):  WCS - 0x%X\n", pData->minor, val16);
      ...
      // LPU
      val16 = readw((unsigned short*)(pData->hwr_chan_addr[0]+0x12));
      if(uceipci_debug >= 2)
        printk(KERN_INFO "uceipci(%d):  LPU - 0x%X\n", pData->minor, val16);
      ver_lpu_major = (val16 & ver_lpu_major) >> 8;
      ver_lpu_minor = val16 & 0x00FF;
      if(val16 >= 0x0411) 
        ver_lpu_build = readw((unsigned short*)(pData->hwr_chan_addr[0]+0x34));
      else
        ver_lpu_build = (val16 & 0xF000) >> 12;
      printk(KERN_INFO "uceipci(%d):  WCS_ver: %d.%02X (build: %d), LPU_ver: %d.%02X (build: %d)\n", pData->minor, ver_wcs_major, ver_wcs_minor, ver_wcs_build, ver_lpu_major, ver_lpu_minor, ver_lpu_build);
    }
...
static int intrpt_cntrl_1553(DEV_DATA* pData, int mode) {
  ...
  if(mode == 0) {
    ...
    if((status = request_irq(pData->irq, uceipci_1553_isr, IRQF_DISABLED|IRQF_SHARED, DRIVER_NAME, (void*)pData)) != 0) {
   #else
    if((status = request_irq(pData->irq, uceipci_1553_isr, SA_INTERRUPT|SA_SHIRQ, DRIVER_NAME, (void*)pData)) != 0) {
   #endif
...
//  2. if using kernel 2.6.21 or later with CONFIG_DEBUG_SHIRQ defined, and
       ...
       if((cntrl_reg >> i) & 0x1) {
         ...
         while(indx_mask <= pData->ids_mask_intrpt) {
           ...
           if((indx_mask & pData->ids_mask_intrpt) && (pId->sigpid > 1)) {
             ...
             ts = pid_task(find_vpid(pId->sigpid), PIDTYPE_PID);
            #else
             ts = find_task_by_pid(pId->sigpid); 
            #endif
             rcu_read_unlock();
            #endif
             if(!ts)
