 ...
 *
 *             This module provides the interface between GE Intelligent
 *             Platforms avionics PCI products and the Linux kernel 2.6.x.  It
 *             performs the necessary functions (registering, memory mapping,..)
 *             needed to operate in a Linux operating system.
 *
 *             This module is utilized by multiple GE Intelligent
 *             Platforms avionics products, use care when performing
 *             modifications.
 * 
 *
...
/* $Revision:  1.20 Release $
   Date        Revision
  ----------   ---------------------------------------------------------------
  03/30/2005   initial.  Support for: Linux kernel 2.6, SYSFS, and Bustools/1553
                and CEI-x20 PCI devices. bch
  07/15/2005   modified to include support for Linux kernels 2.6.9 to 2.6.12:
                removed calls to "cdev_get" and "cdev_put", added 
                "remap_pfn_range", changed condition for return value from
                "pci_register_driver", and redesigned the "read"
                implementation. bch
  09/12/2005   added support for CEI-x30 devices. bch		
  01/12/2006   added 64-bit support. bch
  02/20/2006   added support for the CEI-500 device. bch
  05/25/2006   added signal and sigval to uceiisa_write. bch
  09/13/2006   added support for AMC-1553 and AMC-A30. bch
  03/27/2007   added support for EPMC-1553 and QPCX-1553. fixed reading board
                memory in uceipci_read. bch
  05/30/2007   modified to support multiple processes. added verify_id. added
                intrpt_cntrl. modified uceipci_read to wait for notification
                from the ISR that a hardware interrupt occured. bch
  07/09/2007   added support for the CEI-530. bch
  12/11/2007   added support for the RCEI-830RX and R15-EC. modified
                uceipci_probe. added AMC-1553 to intrpt_cntrl. replaced
                get_current() with current for pids. bch
  01/17/2008   added support for the RAR-CPCI, RAR-EC, P-708, P-SER, P-DIS,
                and P-MIO. bch
  11/18/2008   added support for the R15-AMC. modified uceipci_read when
                reading PCI config space. bch
  02/27/2009   modified uceipci_read by returning the hardware interrupt count
                from the ISR. modified uceipci_probe to probe for BAR regions
                and map all MEM type BAR regions. removed hardware interrupt
                status from intrpt_cntrl. bch 
  07/21/2009   added support for the RXMC-1553. added hardware interrupt
                support for CEI-x30 boards. bch
  10/20/2009   added support for the RPCIe-1553. bch
  ...
  09/27/2011   added support for CEI-430A. bch
...
#include "uceipci_arinc.h"
 ...
 //  struct pci_driver* pdrv = to_pci_driver(drv);
   sscanf(buf,"%d", &uceipci_debug); // atoi 
   return strlen(buf);	
 };
...
static void __exit uceipci_remove(struct pci_dev* pdev) {
  ...
  for(i=0;i<MAX_DEVICES;i++) {
    if(ucei_pdev[i] == pdev) {
      if(uceipci_debug) 
        printk(KERN_INFO "uceipci(%d):  pdev 0x%lx (bus: %d  dev: %d)\n", pdev->device, (unsigned long)ucei_pdev[i], pdev->bus->number, pdev->devfn >> 3);
      ucei_pdev[i] = NULL;
      ucei_pdev_indx--;
      break;
    }  
...
static int __init uceipci_init_module(void) {
 ...
 #if ((LINUX_VERSION_CODE < KERNEL_VERSION(2,6,3)) || (LINUX_VERSION_CODE > KERNEL_VERSION(2,6,39)))
  printk(KERN_WARNING "uceipci:  this driver has not been tested with this kernel version: %d.%d.%d\n", (LINUX_VERSION_CODE&0xFF0000)>>16, (LINUX_VERSION_CODE & 0xFF00)>>8, LINUX_VERSION_CODE&0xFF);
 #endif
  if(uceipci_debug) {
    if(uceipci_debug != 0)
  ...
  if(status == 0) { 
    printk(KERN_ERR "uceipci:  no Condor PCI devices detected.\n");
 #endif
    return status;
  }
