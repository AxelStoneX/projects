...
#define LSP_VERSION 0x0122

// IOCTLS for 2.4 driver
#ifdef KERNEL_24
...
CEI_INT vbtMapBoardAddresses(CEI_UINT device, PDEVMAP_T pMap) {
   ...
   _pMap->llDriverVersion = LSP_VERSION; 
...
CEI_VOID vbtFreeBoardAddress(CEI_UINT device) {
   vbtFreeBoardAddresses(&cei_pMap[device]);   
}
...
CEI_INT vbtMapBoardAddress(CEI_UINT device, CEI_UINT map_len, CEI_CHAR** addr, CEI_VOID* junk, CEI_INT junk2) {
   ...
   if((status = vbtMapBoardAddresses(device, &devMap)) != BTD_OK)
     return status;
   memcpy(&cei_pMap[device], &devMap, sizeof(devMap));

   if(devMap.memSections == 1) i = 0;
   else if(devMap.memSections == 2) i = 1;
   else i = 2;
   addr[0] = (CEI_CHAR*)devMap.memHostBase[i];
