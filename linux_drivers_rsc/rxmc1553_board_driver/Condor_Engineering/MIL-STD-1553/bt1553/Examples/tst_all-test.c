...
API_INT_FIFO  sIntFIFO_BC;     // Thread FIFO structure for BC
API_INT_FIFO  sIntFIFO_RT;     // Thread FIFO structure for RT
API_INT_FIFO  sIntFIFO_BM;     // Thread FIFO structure for BM

BT_INT msg_cnt, uca32;

int main(int argc, char **argv)                     
{
   int status,channel;
   unsigned int pwFlag,cardnum,cardnum1,device;
   int i;
   DeviceList dlist;
...
//   status = BusTools_API_InitExtended(cardnum,0,0,&pwFlag,PLATFORM_PC,Q1041553,NATIVE,CHANNEL_1,CARRIER_MAP_DEFAULT);
//   status = BusTools_API_InitExtended(cardnum,0,0,&pwFlag,PLATFORM_PC,QPCI1553,NATIVE,CHANNEL_1,CARRIER_MAP_DEFAULT);
...
/* This initialization is for the new BusToos_API_OpenChannel function */
   ...
   printf("BusTools_API_OpenChannel status for cardnum %d = %d\n",cardnum,status);
...

   {
     float f1, f2;
     int build;
     status = BusTools_GetFWRevision(cardnum,&f1,&f2,&build);
     printf("F/W Revsion = %f - %f\n",f1,f2);
   }

   {
     BT_U16BIT csc=0, acr=0;     
     status = BusTools_GetCSCRegs(cardnum, &csc, &acr);
     printf("BusTools_GetCSCRegs status = %d\n",status);
     uca32 = (csc & 0x4000) >> 14;
     msg_cnt=0;
   }
   
   status = BusTools_SetInternalBus(cardnum,1);  // Set up bus 0 = External 1 = Internal
   ...
   status = BusTools_TimeTagMode(crdnum, API_TTD_IRIG, API_TTI_IRIG, API_TTM_IRIG, NULL, 0, 0, 0 );
   printf("BusTools_TimeTagMode status = %d\n",status);
...
//   status = BusTools_RegisterFunction(cardnum,&sIntFIFO_BC,0);
//   printf("BusTools_RegisterFunction status = %d\n",status);

//   status = BusTools_RegisterFunction(cardnum,&sIntFIFO_RT,0);
//   printf("BusTools_RegisterFunction status = %d\n",status);

   status = BusTools_RegisterFunction(cardnum,&sIntFIFO_BM,0);
   printf("BusTools_RegisterFunction status = %d\n",status);
...
void setupThe_RT_IntFIFO(int cardnum)
{
   ...
   sIntFIFO_RT.iNotification  = 0;
   sIntFIFO_RT.FilterType     = EVENT_RT_MESSAGE;

   for ( rt=0; rt < 32; rt++ )
      for (tr = 0; tr < 2; tr++ )
         for (sa = 0; sa < 32; sa++ )
            sIntFIFO_RT.FilterMask[rt][tr][sa] =  0xffffffff;  // Disable all Word Counts on all RT/SA/TR combos
		 
   // Call the register function to register and start the thread.
   status = BusTools_RegisterFunction(cardnum, &sIntFIFO_RT, 1);
   printf("BusTools_RegisterFunction status = %d\n",status);
}
...
void setupThe_BC_IntFIFO(int cardnum)
{
   ...
   sIntFIFO_BC.iNotification  = 0;
   sIntFIFO_BC.FilterType     = EVENT_BC_MESSAGE;

   for ( rt=0; rt < 32; rt++ )
      for (tr = 0; tr < 2; tr++ )
         for (sa = 0; sa < 32; sa++ )
            sIntFIFO_BC.FilterMask[rt][tr][sa] =  0xffffffff;  // Enable all Word Counts on all RT/SA/TR combos

/* Use this if to get notified only on No response, late response of message error 
   sIntFIFO_BC.EventInit=USE_INTERRUPT_MASK;   
   for ( rt=0; rt < 32; rt++ )
      for (tr = 0; tr < 2; tr++ )
         for (sa = 0; sa < 32; sa++ )
            sIntFIFO_BC.EventMask[rt][tr][sa] = BT1553_INT_NO_RESP | BT1553_INT_ME_BIT //interrupt on ME and no-rsp
                                              | BT1553_INT_LATE_RESP; // interrupt on Late-response
*/
   // Call the register function to register and start the thread.
   status = BusTools_RegisterFunction(cardnum, &sIntFIFO_BC, 1);
   printf("BusTools_RegisterFunction status = %d\n",status);
}
...
void setupThe_BM_IntFIFO(int cardnum)
{
   ...
   sIntFIFO_BM.iNotification  = 0;
   sIntFIFO_BM.FilterType     = EVENT_BM_MESSAGE;

   for ( rt=0; rt < 32; rt++ )
      for (tr = 0; tr < 2; tr++ )
         for (sa = 0; sa < 32; sa++ )
            sIntFIFO_BM.FilterMask[rt][tr][sa] =  0xffffffff;  // Enable all Word Counts on all RT/SA/TR combos

/********* Interrupt only on message error ***********************************
   sIntFIFO_BM.EventInit=USE_INTERRUPT_MASK;   
   for ( rt=0; rt < 32; rt++ )
      for (tr = 0; tr < 2; tr++ )
         for (sa = 0; sa < 32; sa++ )
            sIntFIFO_BM.EventMask[rt][tr][sa] = BT1553_INT_ME_BIT;  // Enable all Word Counts on all RT/SA/TR combos
*****************************************************************************/
   // Call the register function to register and start the thread.
   status = BusTools_RegisterFunction(cardnum, &sIntFIFO_BM, 1);
   printf("BusTools_RegisterFunction status = %d\n",status);
}
