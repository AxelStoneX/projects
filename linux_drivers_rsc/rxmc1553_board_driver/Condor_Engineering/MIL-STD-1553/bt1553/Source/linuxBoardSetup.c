   ...
   BT_U32BIT phys_addr)
{
   ...
   *********************************************************************/
   if ( (CurrentCardType[cardnum] != QPMC1553)  &&
        ...
        (CurrentCardType[cardnum] != AR15VPX)   &&
        ...
        (CurrentCardType[cardnum] != RAR15XMC)  &&
  ...
  *********************************************************************/
   if ( CurrentPlatform[cardnum] == PLATFORM_PC && CurrentCardType[cardnum] != VME1553)
   {
      if( (status = vbtMapBoardAddresses(phys_addr, &bt_devmap[api_device[cardnum]])) != BTD_OK)
         return status;
      ...
      else if (bt_devmap[api_device[cardnum]].memSections == 2) 
      {
         ...
         bt_iobase[cardnum] = (unsigned long)bt_devmap[api_device[cardnum]].memHostBase[0];
      }
   ...
   *********************************************************************/
    ...
    **********************************************************************/
   if (CurrentCardType[cardnum] == Q1041553 || CurrentCardType[cardnum] == Q1041553P)
   {
   ...
   /*********************************************************************
    * Setup the access parameters for the RAR15-XMC.
    **********************************************************************/
   if (CurrentCardType[cardnum] == RAR15XMC)
   {
      return vbtPageAccessSetupRAR15(cardnum, hostaddr);
   }
   ...
   /*********************************************************************
   *  Setup the access parameters for the VME-1553.
   *********************************************************************/
   if (CurrentCardType[cardnum] == VME1553)
   {
      /* Get the local address of the VME memory space */
      status = vbtMapBoardAddress(phys_addr,0x2000000,
               (void *)&bt_PageAddr[cardnum][0],0,CurrentMemMap[cardnum]);        /* A32 Memory space. */
      if ( status != BTD_OK )
         return (status);
      hostaddr = bt_PageAddr[cardnum][0];

      /* Get the local address of the VME IO space */
      status = vbtMapBoardAddress(bt_iobase[cardnum],0x40,
               (void *)&bt_PageAddr[cardnum][1],0,CARRIER_MAP_A16);              /* A16 Memory space. */
      if ( status != BTD_OK )
         return (status);
      bt_iobase[cardnum] = (BT_U32BIT)bt_PageAddr[cardnum][1];
      return vbtPageAccessSetupVME(cardnum, (unsigned long)phys_addr, hostaddr);
   }
#endif
...
int vbtOpen1553Channel(UINT *chnd, UINT mode, int devid, UINT channel) {
   int status, cardnum;
   unsigned int deviceID, chan_cnt;

   cardnum = deviceID = chan_cnt = 0;
   ...
   *chnd = cardnum;
   ...
   if(CurrentCardType[cardnum] == IPD1553) {       // Card type is IP-D1553.
     CurrentCarrier[cardnum]  = IPD1553_PCI;       // PCI Carrier.
     CurrentMemMap[cardnum]   = CARRIER_MAP_LARGE; // Set large memory map
   }
...
NOMANGLE BT_INT CCONV BusTools_ListDevices(DeviceList* dlist) {
   ...
   for(index=0;index<dev_num;index++) {
      ...
      case PCCD1553:
        ...
        break;
      case CPCI1553:
        strcpy(dev_name,"cPCI-1553");
        break;
//      case QPM1553:
//      case AMC1553:
      case QPMC1553:
        strcpy(dev_name,"QPMC-1553 / QPM-1553 / AMC-1553");
        break;
      case QPCI1553:
