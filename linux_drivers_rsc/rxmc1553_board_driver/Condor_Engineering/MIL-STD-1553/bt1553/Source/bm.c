 ...
 *    BusTools_BM_MessageAlloc   - alloc and init BM Message Buffers/alloc memory
 ...
 *
 * INTERNAL ROUTINES:
 *    BM_V5MessageConvert        - convert raw BM msg buf to API_BM_MBUF struct F/W v5/4
 *    BM_V6MessageConvert        - convert raw BM msg buf to API_BM_MBUF struct F/W v6
 ...
 *    v5_BM_MessageAlloc         - alloc and init BM Message Buffers F/W v5/4
 *    v6_BM_MessageAlloc         - alloc memory for variable size BM buffers F/W v6
   ...
   API_BM_CBUF * api_cbuf)
{
   ...
   *******************************************************************/
   // Get address of current entry in fbuf.
   addr_fbuf = BTMEM_BM_V6FBUF +        // Byte addr of the beginning of fbuf.
       (rtaddr << 8) + (tr << 7) + (subaddr << 2 ); // Byte offset of entry.
   ...
   *******************************************************************/
        ...
        (api_cbuf->t.wcount == 0x00000000L) )
   {
      if ( waddr != BTMEM_BM_V6CBUF_DEF + BM_CBUF_SIZE )
      {
         ...
         // Update CBUF's address in FBUF
         // The byte address in FBUF for this CBUF is in "addr_fbuf"
         waddr = BTMEM_BM_V6CBUF_DEF + BM_CBUF_SIZE;
         vbtWriteRAM32[cardnum](cardnum, &waddr, addr_fbuf, 1);
      }
      return API_SUCCESS;
   }
   ...
   *******************************************************************/
   ...
   if ( waddr != BTMEM_BM_V6CBUF_DEF )  // If not the default,
   {
      /*******************************************************************
      *  Store new cbuf in hardware.
      *******************************************************************/
   ...
   else
   {
      ...
      *******************************************************************/
      ...
      // Update CBUF's address in FBUF
      // The byte address in FBUF for this CBUF is in "addr_fbuf"
      waddr = addr;  // Word address of the new CBUF.
      vbtWriteRAM32[cardnum](cardnum, &waddr, addr_fbuf, 1);
   ...
   BT_UINT enable_b)
{
   ...
   return BT_BM_Init[cardnum](cardnum,enable_a,enable_b);
}
   ...
   BT_U32BIT enable)
{
   ...
   ************************************************/
   ...
   if(channel_is_shared[cardnum])
   {
      vbtReadRAM[cardnum](cardnum,(BT_U16BIT *)&btmem_bm_mbuf_next[cardnum],BTMEM_CH_SHARE + SHARE_BTMEM_BM_MBUF_NEXT,4*2);
      vbtReadRAM[cardnum](cardnum,(BT_U16BIT *)&btmem_pci1553_next[cardnum],BTMEM_CH_SHARE + SHARE_BTMEM_PCI1553_NEXT,4*2);
   } 
   ...
   *******************************************************************/
   ...
   {
      vbtWriteRAM8(cardnum,(LPSTR)&bm_inited[cardnum],BTMEM_CH_SHARE + SHARE_BM_INITED,1);
      vbtWriteRAM[cardnum](cardnum,(BT_U16BIT *)&btmem_bm_mbuf_next[cardnum],BTMEM_CH_SHARE + SHARE_BTMEM_BM_MBUF_NEXT,4);
      vbtWriteRAM[cardnum](cardnum,(BT_U16BIT *)&btmem_pci1553_next[cardnum],BTMEM_CH_SHARE + SHARE_BTMEM_PCI1553_NEXT,4);
   } 
...
*     or identified. The space allocated is the buffer count muliplied by the size
   ...
   API_BM_MBUF * mbuf_user)     // (o) Pointer to user-supplied output buffer
{
   ...
   *******************************************************************/
   ...
   BM_HEADER      bmheader;
#ifdef USE_BM_DMA
   BM_V6MBUF        *mbuf_PCI;   // Pointer to the RAW data
#endif
   union{
     BM_V6MBUF   bm_mbuf; 
...
#ifdef USE_BM_DMA
   mbuf_PCI = (BM_V6MBUF *)mbuf_hw_offset;
#else
   ...
   *******************************************************************/
   ...
   for(i=0; i<count; i++)
   {
      ...
      mbuf_user->value[i] = bm.bm_mbuf.data[i].value;
   ...
   API_BM_MBUF * api_mbuf)
{
   ...
   *******************************************************************/
   if(board_is_v5_uca[cardnum]) 
   {
      ...
      BM_V5MessageConvert(cardnum, (BT_U32BIT)bt_devmap[cardnum].laddr, api_mbuf);
#else
      /*******************************************************************
   ...
   else
   {
      ...
      vbtSetPLXRegister32(cardnum,PLX_DMA_BOARD_ADDR0, addr+0x100000);
      vbtSetPLXRegister8(cardnum,PLX_DMA_STATUS0, 0x3);
      BM_V6MessageConvert(cardnum, (BT_U32BIT)bt_devmap[cardnum].laddr, api_mbuf);
#else
   ...
   *******************************************************************/
   ...
   while ( bmaddr != bmaddr_cur )       // While SW ptr != HW ptr ...
   {
      ...
      if ( oflowaddr == NULL )
      {
         ...
         vbtSetPLXRegister8(cardnum,PLX_DMA_STATUS0, 0x3);
         BM_V5MessageConvert(cardnum, (BT_U32BIT)bt_devmap[cardnum].laddr, useraddr);
#else
   ...
   *******************************************************************/
   ...
   while ( bmaddr != bmaddr_cur )       // While SW ptr != HW ptr ...
   {
      ...
      if ( oflowaddr == NULL )
      {
         ...
         vbtSetPLXRegister32(cardnum,PLX_DMA_BOARD_ADDR0, bmaddr+0x100000);
         vbtSetPLXRegister8(cardnum,PLX_DMA_STATUS0, 0x3);
         msg_size = BM_V6MessageConvert(cardnum, (BT_U32BIT)bt_devmap[cardnum].laddr, useraddr);
#else
...
/****************************************************************************
*
*  PROCEDURE - BusTools_InterMessageGap2
*
*  FUNCTION
*     This routine computes the intermessage gap which exists between two
*     specified messages.  This is accomplished by first differencing the
   ...
   API_BM_MBUF * second)
{
   ...
   *******************************************************************/
   if(board_is_v5_uca[cardnum])
      ...
      dInterMsgGap = second->time.microseconds/1000 - first->time.microseconds/1000;
   ...
   FILE  * hfMemFile)       // (i) handle of output file
{
   ...
   *******************************************************************/
   ...
   while ( bmaddr != bmaddr_end )       // While not at the end of the buffer
   {
      ...
      fprintf(hfMemFile, " TTH 0x%08X TTL 0x%08X DIFF 0x%08X (%0.3f usecs)\n",
          useraddr.time.topuseconds, useraddr.time.microseconds, diff, difff/1000);
...
/****************************************************************************
*
*   PROCEDURE NAME - v5_RT_ReadNextMessage()
*
*   FUNCTION
...
/****************************************************************************
*
*   PROCEDURE NAME - v6_RT_ReadNextMessage()
*
*   FUNCTION
*       This function reads the next RT message that meets the criteria set
*       by the passed parameters.  The arguments include a timeout value in
