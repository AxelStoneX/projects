...
//C0   
    "*BMStBuf", "*BMEndBf", "*BMHead ", "rC3     ", "BMFltBas", "*BMTrgBf", "BM IntEn", "rC7     ", 
    "rC8     ", "rC9     ", "rCA     ", "rCB     ", "rCC     ", "rCD     ", "rCE     ", "rCF     ", 
//D0
    "rD0     ", "rD1     ", "rD2     ", "rD3     ", "PBCntrSt", "rD5     ", "rD6     ", "rD7     ", 
   ...
   char * heading)          // (i) pointer to message to display in file                      
{
   ...
   *******************************************************************/
   ...
   if ( CurrentCardType[cardnum] == QPCI1553 )
      ...
      fprintf(hfMemFile, " RAR15-XMC-XT WCS V%d, FPGA V%x, Channel %d",
              _HW_WCSRev[cardnum], _HW_FPGARev[cardnum], CurrentCardSlot[cardnum]+1);
   ...
   fprintf(hfMemFile, " bt_PageAddr[cardnum][0..6] = %5.5X %5.5X %5.5X %5.5X\n IO-Base = %X\n",
           (BT_U32BIT)(bt_PageAddr[cardnum][RAM_PAGE]), (BT_U32BIT)bt_PageAddr[cardnum][HWREG_PAGE],
           (BT_U32BIT)bt_PageAddr[cardnum][TTREG_PAGE], (BT_U32BIT)bt_PageAddr[cardnum][CSC_REG_PAGE], bt_iobase[cardnum]);
...
#ifndef FILE_SYSTEM
      fflush(stdout); 
      printf("\nHit Enter for next data. 'q' to exit dump...\n");
      if(getchar()== 'q')
    	  break;
      fflush(stdin);
#endif      
