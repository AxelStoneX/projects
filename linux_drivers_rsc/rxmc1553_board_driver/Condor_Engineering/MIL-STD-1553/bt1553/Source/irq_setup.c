...
// polling timer
#define TIMER_STOP  3
#define POLLING_TIMER_THREAD_PRIORITY 99
...
// hardware interrupts
#define HWINT_THREAD_PRIORITY 2
...
int vbtInterruptSetup(BT_UINT cardnum, BT_INT hw_int_enable, BT_INT api_dev) {
   ...
   if(conf_cardnum[cardnum].mode >= API_HW_INTERRUPT) {
     ...
     switch(CurrentCardType[cardnum]) {
      ...
      case Q1041553:
      case ISA1553:
        if((irq = vbtGetInterrupt(api_dev)) == 0)
          return BTD_BAD_HW_INTERRUPT;
        // Load the interrupt control register with the interrupt number.
        *((BT_U16BIT*)(bt_PageAddr[cardnum][3] + 0xC)) = (1 << irq);
        if(board_is_v5_uca[cardnum])
          vbtSetHWRegister(cardnum, HWREG_WRITE_INTERRUPT, 1);  // set the interrupt write register
        else
          vbtSetRegister[cardnum](cardnum, HWREG_WRITE_INTERRUPT, 1);   // set the interrupt write register
        break;
      default:
     ...
     if(HwIntSig[api_dev].status == 0) {
       ...
       if((status = CEI_THREAD_CREATE(&HwIntSig[api_dev].thrd, HWINT_THREAD_PRIORITY, func_HwIntSig, &api_dev)) != 0)
         return POSIX_ERROR;
     ...
     if(HwIntWQ[cardnum].status == 0) {
       ...
       if((status = CEI_THREAD_CREATE(&HwIntWQ[cardnum].thrd, HWINT_THREAD_PRIORITY, func_HwIntWQ, &cardnum)) != 0)
         return POSIX_ERROR;
...
void vbtInterruptClose(BT_UINT cardnum) {
  ...
  if(conf_cardnum[cardnum].mode >= API_HW_INTERRUPT) {
    if((CurrentCardType[cardnum] != PCI1553)   &&
       ...
       (CurrentCardType[cardnum] != QPM1553)   &&  // includes QPMC1553 and AMC1553
  ...
  // lock the mutex to prevent the polling timer from aquiring the lock, then destroy the mutex
  if(conf_cardnum[cardnum].mode < API_HW_ONLY_INT) {
