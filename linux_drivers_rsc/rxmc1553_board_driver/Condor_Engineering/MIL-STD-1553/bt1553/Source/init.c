 ...
 *     BusTools_InputTriggerConfig   - Configure discrete or differential as input trigger for a channel
 ...
 *     BusTools_Set1553Mode      - Set 1553A or 1553b mode by RT address
...
NOMANGLE BT_INT CCONV BusTools_API_ShareChannel(BT_UINT cardnum)     // (i) 
{
   ...
   vbtReadRAM32[cardnum](cardnum,(BT_UINT *)&header,BTMEM_CH_V6SHARE,1);
   ...
   vbtWriteRAM[cardnum](cardnum,(BT_U16BIT *)&cshare[cardnum],BTMEM_CH_V6SHARE,CHANNEL_SHARE_WSIZE);
...
NOMANGLE BT_INT CCONV BusTools_API_JoinChannel(BT_UINT  *chnd, BT_UINT device, BT_UINT channel)     // (i) 
{
   ...
   *chnd = cardnum;
   ...
   bt_PageAddr[cardnum][HWREG_PAGE]    = hostaddr + REG_OFFSET + (REG_SIZE * CurrentCardSlot[cardnum]) + HWREG_OFFSET;
   bt_PageAddr[cardnum][TTREG_PAGE]    = hostaddr + REG_OFFSET + (REG_SIZE * CurrentCardSlot[cardnum]) + TTREG_OFFSET;
   bt_PageAddr[cardnum][TRIG_PAGE]     = hostaddr + REG_OFFSET + (REG_SIZE * CurrentCardSlot[cardnum]) + TRIG_OFFSET;
   bt_PageAddr[cardnum][RAM_PAGE]      = hostaddr + RAM_OFFSET[cardnum] + (RAM_SIZE[cardnum] * CurrentCardSlot[cardnum]);

   vbtReadRAM[cardnum](cardnum,(BT_U16BIT *)&cshare[cardnum],BTMEM_CH_V6SHARE,CHANNEL_SHARE_WSIZE);
   ...
   nBM_MBUF_len[cardnum] = (BM_MBUF_SIZE+15) & ~0x000F;   // Pad to multiple of 8 words
   ...
   *******************************************************************/
   ...
   vbtReadRAM32[cardnum](cardnum,&scount,BTMEM_CH_V6SHARE+SHARE_COUNT,1);
   scount++;
   vbtWriteRAM32[cardnum](cardnum,&scount,BTMEM_CH_V6SHARE+SHARE_COUNT,1);
...
NOMANGLE BT_INT CCONV BusTools_API_QuitChannel(BT_UINT cardnum, BT_UINT qFlag)     // (i) 
{
   ...
   if(qFlag & RT_QUIT)
   {
      ...
      vbtWriteRAM32[cardnum](cardnum,&rt_running[cardnum],BTMEM_CH_V6SHARE + SHARE_RT_RUNNING,1);
   }
   if(qFlag & BM_QUIT)
   {
      bm_inited[cardnum] = 0;
      ...
      vbtWriteRAM32[cardnum](cardnum,&bm_running[cardnum],BTMEM_CH_V6SHARE + SHARE_BM_RUNNING,1);
   }
   if(qFlag & BC_QUIT)
   {
      bc_inited[cardnum] = 0;
      bc_running[cardnum] = 0;
      vbtWriteRAM32[cardnum](cardnum,&bc_inited[cardnum],BTMEM_CH_V6SHARE + SHARE_BC_INITED,0);
      vbtWriteRAM32[cardnum](cardnum,&bc_running[cardnum],BTMEM_CH_V6SHARE + SHARE_BC_RUNNING,0);

   }
   //Update the share count
   vbtReadRAM32[cardnum](cardnum,(BT_U32BIT *)&scount,BTMEM_CH_V6SHARE+SHARE_COUNT,1);
   scount--;
   vbtWriteRAM32[cardnum](cardnum,(BT_U32BIT *)&scount,BTMEM_CH_V6SHARE+SHARE_COUNT,1);

   if(qFlag & SHARE_QUIT)
   {
      cshare[cardnum].header = CHANNEL_NOT_SHARED;
      vbtWriteRAM32[cardnum](cardnum,(BT_U32BIT *)&cshare[cardnum].header,BTMEM_CH_V6SHARE,1);
      channel_is_shared[cardnum] = 0;
   }
   ...
   int       *first_time)      // (i) State information, see above
{
   ...
   case 0:               /* Open the output file and fall into case 1   */
      ...
      if ( CurrentCardType[cardnum] == PCI1553 )
         ...
         fprintf(hfMemFile, "RAR15-XMC-XT channel[1/2/3/4] = %d\n\n",
                            CurrentCardSlot[cardnum]);
   ...
   case 0:               /* Open the output file and fall into case 1   */
      ...
      if ( CurrentCardType[cardnum] == PCI1553 )
         ...
         printf("RAR15-XMC-XT channel[1/2/3/4] = %d\n\n",
                            CurrentCardSlot[cardnum]);
	  else if ( CurrentCardType[cardnum] == R15XMC2 )
...
*
**********************************************************************/
#define MEM_TST_V6PG   256              /* Sixe of a memory test block */
 ...
 //  setReadWrite(cardnum);
   ...
   *******************************************************************/
   ...
   if(bt_op_mode[cardnum] == 0x40)
   {
      ...
      {
         api_sethwcbits(cardnum,0x40);    //Set 1553a
   ...
   else
   {
      ...
      if(board_is_v5_uca[cardnum])
      {
         ...
         if((_HW_FPGARev[cardnum] & 0xfff) >= 0x506)   
         {  
            vbtSetHWRegister(cardnum,HWREG_RT_MODE1,0x0); //Set all RT addresses for B mode
      ...
      else
      {
         ...
         api_clearhwcbits(cardnum, 0x40);     //reset 1553a
...
*       and the number of regisers to read (not byte count).  All registers are 
...
*       Pass the apporpriate size array pointer to hold the 16- or 32-bit result.
*
*  RETURNS
*       API_SUCCESS -> success
...
/****************************************************************************
*
*  PROCEDURE NAME -   BusTools_ReadSharedMemory
*
*  FUNCTION
...
NOMANGLE BT_INT CCONV BusTools_ReadSharedMemory(
   BT_UINT cardnum,          // (i) card number
...
NOMANGLE BT_INT CCONV BusTools_ReadBoardTemp(
   ...
   BT_UINT location,       // (i) Temp location INTERNAL,FPGA,ZBT,CH4_1553,MID_BOARD
   BT_INT *tmp)            // (o) temperature
{
   ...
   *******************************************************************/
   ...
   if(board_is_v5_uca[cardnum])
   {
      vbtWriteHIF(cardnum,(LPSTR)&tdata,0x160,2);
      MSDELAY(1);
      vbtReadHIF(cardnum,(LPSTR)&tdata,0x160,2);
   }
   else
   {
      vbtSetCSCRegister[cardnum](cardnum,GLBREG_TMP_SEN_WRITE,tdata);
      MSDELAY(1);
...
NOMANGLE BT_INT CCONV BusTools_BoardIsMultiFunction(BT_UINT cardnum)
{
   ...
   else
   {
      ...
      else if(board_is_dual_function[cardnum])
   ...
   BT_INT  * build)          // (o) pointer to lpu revision number
{
   ...
   *******************************************************************/
   ...
   else
   {
      ...
      *lrev = (float)( ((((temp &0x0f00)>> 8)*100) + (((temp & 0x00f0)>> 4)*10) + (temp & 0x000f)))/(float)100.0;
      *build = vbtGetRegister[cardnum](cardnum,HWREG_BUILD_NUMBER);
   }
...
NOMANGLE BT_INT CCONV BusTools_DumpRegisters(
   BT_UINT cardnum,         // (i) card number
   BT_U32BIT * buff)             // (i) user buffer pointer
{ 
   int i;
   /*******************************************************************
   *  Do error checking
   ...
   *******************************************************************/
   for(i=0;i<0x100;i++)
      buff[i] = vbtGetRegister[cardnum](cardnum, i);
   ...
   *******************************************************************/
   ...
   switch(region)
   {
      ...
      case HWREG:
         ...
         {
            if((addr > HWREG_V6COUNT*2) || (addr+bcount > HWREG_V6COUNT*2))
               return API_BAD_PARAM;
...
/****************************************************************************
*
*  PROCEDURE NAME -   BusTools_WriteSharedMemory
*
*  FUNCTION
...
NOMANGLE BT_INT CCONV BusTools_WriteSharedMemory(
   BT_UINT cardnum,         // (i) card number
                           ...
                           //     Disable = 0;
{
   ...
   *******************************************************************/
   ...
   /*******************************************************************
   * Set the Enable interrupt on Minor Frame Overflow selection
   *******************************************************************/
   if(intflag & MFO_INTR_ENBL)
      wFWControlValue |= FW_MINORFRAME_OFLOW_ENA;
   else
      wFWControlValue &= ~FW_MINORFRAME_OFLOW_ENA;
...
NOMANGLE BT_INT CCONV BusTools_PCI_Reset(BT_INT cardnum, BT_UINT reset_flag)
{
   ...
   {
      if(reset_flag)
          ((BT_U16BIT *)(bt_PageAddr[cardnum][CSC_REG_PAGE]))[9] = flipws(0x1); //Enable PCI Reset
      else
         ((BT_U16BIT *)(bt_PageAddr[cardnum][CSC_REG_PAGE]))[9] = 0x0; //Disable PCI Reset
   }
...
NOMANGLE BT_INT CCONV BusTools_VME_Reset(BT_INT cardnum, BT_UINT reset_flag)
{
   ...
   {
      if(reset_flag)
         ((BT_U16BIT *)(bt_PageAddr[cardnum][CSC_REG_PAGE]))[9] = flipws(0x1); //Enable VME Reset
       else
         ((BT_U16BIT *)(bt_PageAddr[cardnum][CSC_REG_PAGE]))[9] = 0x0; //Disable VME Reset
   }
   ...
   BT_UINT  coupling)      // (i) 0 -> direct, 1 -> transformer coupling
{
   ...
   *******************************************************************/
   ...
   else
   {
      ...
      vbtWriteGLBR[cardnum](cardnum,&voltword,CREG_DAC_CONTROL,1);
   }
...
char* getStatusString(BT_INT status)        // (i) API Status code{(
{
   ...
   case API_BUSTOOLS_DWORD_SIZE : return "CEI-WARN -- DWORD byte count must be mulitple of 4.";
   ...
   case API_BC_MULTI_BUFFER_ERR : return "CEI_WARN -- Using wrong function to setup Bus Controller.  Use BusTools_BC_BufferAlloc()";
      ...
      BT_INT status)        // (i) API Status code
{
   ...
   if(status == API_INSTALL_ERROR)
      ...
      sprintf(static_buf, "%s", getStatusString(status)); 
