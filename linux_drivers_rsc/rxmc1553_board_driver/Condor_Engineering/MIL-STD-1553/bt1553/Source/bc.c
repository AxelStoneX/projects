 ...
 *    BusTools_BC_BufferAlloc    - Allocates a single message buffer with 1 or more data buffers
 ...
 *    BusTools_BC_GetBufferCount - Returns the number of buffer allocated for a BC Message V6 Only
 ...
 *    BusTools_BC_MessageBlockAlloc - Allocates memory for a BC Message Buffer (for Multiple BC buffer in F/W V6)
 ...
 *    BusTools_BC_MessageBufferRead - Read a BC message buffer for V6 using data buffer addr
 ...
 *    BusTools_BC_DataBufferWrite  - Update specified BC Message Block data area buffer
 *    BusTools_BC_ReadDataBuffer - Read data from a specified buffer address V6 Only
 *    BusTools_BC_MessageUpdateBuffer - Update specified BC Message Buffer data (for Multi-buffer)
 *    BusTools_BC_DataBufferUpdate - Update specified BC data buffer (V6 Only)
 *    BusTools_BC_SelectBufferRead - Read data from a message selected data buffer V6 Only
 *    BusTools_BC_SelectBufferUpdate - Updates the Data in the selected data buffer for a message V6 Only
   ...
   BT_UINT   num_buffers)   // (i) number of BC message buffers ( 1 or 2 for legacy)
{
   ...
   *******************************************************************/
   ...
   {
      vbtReadRAM32[cardnum](cardnum,&bc_inited[cardnum],BTMEM_CH_V6SHARE + SHARE_BC_INITED,1);
   ...
   *******************************************************************/
   ...
   if(channel_is_shared[cardnum])
      vbtWriteRAM32[cardnum](cardnum,&bc_inited[cardnum],BTMEM_CH_V6SHARE + SHARE_BC_INITED,1); 
...
*     function is only compatible with Multi-BC buffer mode. Call BusToos_BC_init
...
*     associated data buffer at one time.  Each message blcok has a variable 
                            ...
                            // message buffer
{
   ...
   *******************************************************************/
   ...
   if(channel_is_shared[cardnum])
      vbtReadRAM[cardnum](cardnum,(BT_U16BIT *)&btmem_pci1553_next[cardnum],BTMEM_CH_V6SHARE + SHARE_BTMEM_PCI1553_NEXT,2);
   ...
   if(mblock_addr[cardnum] == NULL)
   {
      ...
      mblock_addr[cardnum] = (BT_U32BIT *)malloc(sizeof(BT_U32BIT)); //allocate table of buffer address
   }
   else
   {
      bc_mblock_count[cardnum]++;  //increment the mblock count for this buffer
      mblock_addr[cardnum] = (BT_U32BIT *)realloc((void *)mblock_addr[cardnum], (sizeof(BT_U32BIT) * bc_mblock_count[cardnum]));
   }
   /*******************************************************************
   ...
   *******************************************************************/
   memset(&bc_buffer, 0, sizeof(BC_MSGBUF));
   ...
   ****************************************************************/
   ...
   {   
      ...
      vbtWriteRAM32[cardnum](cardnum, (BT_U32BIT *)&bc_data,addr_data1 + (BC_MSGDATA_SIZE * i), BC_MSGDATA_SIZE);
   }
   mblock_addr[cardnum][bc_mblock_count[cardnum]-1] = nextbc;  
   nextbc = addr;

   if(channel_is_shared[cardnum])
      vbtWriteRAM32[cardnum](cardnum,(BT_U32BIT *)&btmem_pci1553_next[cardnum],BTMEM_CH_V6SHARE + SHARE_BTMEM_PCI1553_NEXT,wsizeof(BT_U32BIT));
   ...
   BT_UINT count)           // (i) Number of BC messages to allocate
{
   ...
   *******************************************************************/
   ...
   if(channel_is_shared[cardnum])
      vbtReadRAM32[cardnum](cardnum,(BT_U32BIT *)&btmem_pci1553_next[cardnum],BTMEM_CH_SHARE + SHARE_BTMEM_PCI1553_NEXT,sizeof(BT_U32BIT));
   ...
   *******************************************************************/
   ...
   if(channel_is_shared[cardnum])
      vbtWriteRAM32[cardnum](cardnum,&btmem_pci1553_next[cardnum],BTMEM_CH_SHARE + SHARE_BTMEM_PCI1553_NEXT,sizeof(BT_U32BIT));
   ...
   BT_UINT count)           // (i) Number of BC messages to allocate
{
   ...
   *******************************************************************/
   ...
   /* Compute the number of bytes required for message and data buffers */
   bytes_required = count * (sizeof(BC_MSGBUF) + (bc_num_dbufs[cardnum] * sizeof(BC_MSGDATA)));

   mblock_addr[cardnum] = (BT_U32BIT *)malloc(count * sizeof(BT_U32BIT)); //allocate table of buffer address
   ...
   *******************************************************************/
   ...
   for ( i = 0; i < count; i++ )
   {
      ...
      ****************************************************************/
      /* Set data buffer addresses */
      addr_data1 = addr_data2 = nextbc + sizeof(BC_MSGBUF);
      bc_buffer.data_addr = RAM_ADDR(cardnum,addr_data1); 
      bc_buffer.messno = i;
   ...
   BT_U16BIT * api_control_word) // (i) Pointer variable to hold control word
{
   ...
   ************************************************/
   ...
   if ( (control_word & 0x0007) == 0x0002 || (control_word & 0x0007) == 0x0003 )  /* Normal 1553 message */
   {
      if ( (control_word & 0x0007) == 0x0003)
      ...
      if (control_word & 0x0020)
         *api_control_word             |= BC_CONTROL_RTRTFORMAT;
      if (control_word & 0x0400)
                            ...
                            //           0 -> Do not test for BC executing msg.
{
   ...
   ************************************************/
   ...
   {
      msg_addr = BC_MBLOCK_ADDR(cardnum, messageid); 
      vbtReadRAM[cardnum](cardnum, &bc_cntl_wrd, msg_addr,1*2);
   }
   else
   { 
      msg_addr = mblock_addr[cardnum][messageid];
      vbtReadRAM[cardnum](cardnum, (BT_U16BIT *)&bc_cntl_wrd, msg_addr + BC_CTLWD_OFFSET, 1);
   }
   ...
   // Update the message control word on the board.
   if(board_is_v5_uca[cardnum])
      vbtWriteRAM[cardnum](cardnum, &bc_cntl_wrd, msg_addr, 1);
   else 
      vbtWriteRAM[cardnum](cardnum, &bc_cntl_wrd, msg_addr + BC_CTLWD_OFFSET, 1);
   ...
   API_BC_MBUF * api_message)  // (i) Pointer to buffer to receive msg
{
   ...
   *******************************************************************/
   ...
   if ( (cwdata & 0x0007) == 0x0002 || (cwdata & 0x0007) == 0x0003 )  /* Normal 1553 message */
   {
      ...
      vbtReadRelRAM32[cardnum](cardnum, (BT_U32BIT *)&dblock, mblock.data_addr,BC_MSGDATA_DWSIZE);
      if ( (mblock.control_word & 0x0007) == 0x0003)
         api_message->control                = BC_CONTROL_MESSAGE;
      else
         api_message->control                = BC_CONTROL_MSG_NOP;
      ...
      case 0x0006:       /* Conditional Branch */
         ...
         else  //
         {
            ...
            else if (addr == addr_prev + BC_DATA_MSTAT1_OFFSET)
            {
               ...
               wordno = 4 + (addr - (addr_prev + BC_DATA_DATA_OFFSET)/2);                       
               if (wordno > 35)
                  return API_BC_BAD_COND_ADDR;
   ...
   API_BC_MBUF * api_message)  // (i) Pointer to buffer to receive msg
{
   ...
   *******************************************************************/
   ...
   union
   {
      BC_MSGBUF       mblock;      // Local copy of hardware BC Message 
      BC_CTRLBUF      cblock;         
      BT_U32BIT       cdata[wsizeof(BC_MSGBUF)];
   }CTRL;
   ...
   union
   {
      BC_MSGDATA      dblock;
      BC_CTRLDATA     cdblock;
      BT_U32BIT       cdata[wsizeof(BC_MSGDATA)];    
   }MDATA;
   ...
   ********************************************************************/
   ...
   if ( (cwdata & 0x0007) == 0x0002 || (cwdata & 0x0007) == 0x0003 )  /* Normal 1553 message */
   {
      ...
      if (CTRL.mblock.control_word & BC_HWCONTROL_BUFFERB)
         api_message->control          |= BC_CONTROL_BUFFERB;
      else
         api_message->control          |= BC_CONTROL_BUFFERA;
      ...
      if (CTRL.mblock.control_word & BC_HWCONTROL_BUFFERB)
         for(i=0; i<count; i++)
            api_message->data[1][i] = MDATA.dblock.data_word[i];
      else
         for(i=0; i<count; i++)
   ...
   BT_U16BIT * buffer)      // (o) pointer to user's data buffer
{
   ...
   *******************************************************************/
   ...
   {
      daddr = mblock.data_addr + (buffer_id * sizeof(BC_MSGDATA));
...
/****************************************************************************
*
*  PROCEDURE NAME - BusTools_BC_MessageReadDataBuffer() V6 Only
*
*  FUNCTION
*     This routine reads the data area of the specified message
*     block.  The data area to be read (there are two for each message
   ...
   API_BC_MBUF * api_message)  // (i) pointer to user's buffer containing msg
{
   ...
   ********************************************************************/
   ...
   BT_U32BIT  bc_msg_addr;   /* Byte address of the current BC message block */
   BT_U32BIT  data_addr;     /* First BC message data buffer byte address    */ 
   ...
   *******************************************************************/
   ...
   if (messtype == BC_CONTROL_MESSAGE || messtype == BC_CONTROL_MSG_NOP)
   {
      ...
      // Set teh data address base on selected bufffer
      if (api_message->control & BC_CONTROL_BUFFERB)
         data_addr = RAM_ADDR(cardnum,bc_msg_addr + BC_MSGBUF_SIZE + BC_MSGDATA_SIZE);  //This points to the second buffer (BUFFER B)

      if (api_message->control & BC_CONTROL_BUFFERA)
         data_addr = RAM_ADDR(cardnum,bc_msg_addr + BC_MSGBUF_SIZE);                    //This points to the first buffer (BUFFER A)  

      mblock.data_addr = data_addr;   //This write data to the start data address only
      ...
      if(board_using_msg_schd[cardnum])
      {
         ...
         mblock.start_frame = api_message->start_frame;
         flip((BT_U32BIT *)&api_message->rep_rate);
      }
      ...
      if(channel_using_multiple_bc_buffers[cardnum])
      {
         vbtReadRelRAM32[cardnum](cardnum,&dblock.next_data,mblock.data_addr + BC_DATA_NEXT_OFFSET,1);
      }
      else
      ...
      vbtWriteRelRAM32[cardnum](cardnum,(BT_U32BIT *)&dblock,data_addr,BC_MSGDATA_DWSIZE);
   ...
   BT_U16BIT * buffer)      // (i) pointer to user's data buffer
{
   ...
   *******************************************************************/
   ...
   {
      daddr = mblock.data_addr + sizeof(BC_MSGDATA);
   }
   else
   {
      daddr = mblock.data_addr - sizeof(BC_MSGDATA);
   ...
   BT_U32BIT frame_time)    // (i) New Frame Time in uSecs
{
   ...
   *******************************************************************/
   ...
   else
   {
      ...
      vbtSetRegister[cardnum](cardnum, HWREG_MINOR_FRAMEV6, frametime32[cardnum]);
   }
   return status;
}
   ...
   FILE  * hfMemFile)       // (i) handle of output file
{
   ...
   fprintf(hfMemFile, "\n%s.Start %8.8X End %8.8X (Byte Offset)\n",
      BusTools_GetAddrName(GETADDR_BCMESS), RAM_ADDR(cardnum,first), RAM_ADDR(cardnum,last));
      ...
      {   // NOOP, STOP or Conditional
         ...
         {  // NOOP Message
            if((CTRL.cblock.control_word & BC_HWCONTROL_SET_TNOP) == BC_HWCONTROL_SET_TNOP)
               fprintf(hfMemFile, "TNop %3d @%08X gap%d:",  CTRL.mblock.messno, RAM_ADDR(cardnum,i),CTRL.cblock.gap_time);
            else
               fprintf(hfMemFile, "Nop  %3d @%08X:", CTRL.cblock.messno, RAM_ADDR(cardnum,i));
         }
         else if ( (CTRL.cblock.control_word & 0x0006) == 0x0004 )
         {  // STOP Message
            fprintf(hfMemFile, "Stp  %3d @%08X:", CTRL.cblock.messno, RAM_ADDR(cardnum,i));
         }
         ...
         fprintf(hfMemFile, " nxt msg@%8.8X\n", CTRL.cblock.addr_next);  // Next message addr
