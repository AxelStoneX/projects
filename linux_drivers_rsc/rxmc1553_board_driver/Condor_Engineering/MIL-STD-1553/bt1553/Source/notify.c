...
static int            nFIFO[MAX_BTA];    // Number of user registered functionqs.
                                                ...
                                                BT_UINT wFlag)
{
   ...
   *  Local variables
   *******************************************************************/
   BT_INT      iFIFO;     // Index to current entry in the pFIFO array
   BT_INT      MaxWait;   // Time out waiting for user function to exit
//   BT_INT      status;
   BT_INT      timeOutDelay;
   ...
   *******************************************************************/
   ...
   if(CEI_THREAD_CREATE(&sIntFIFO->hThread, sIntFIFO->iPriority,(void *)LocalInterruptThreadFunction,(void *)sIntFIFO) != BTD_OK) {
     CEI_EVENT_DESTROY(&sIntFIFO->hEvent);
     CEI_MUTEX_DESTROY(&sIntFIFO->mutex);
     return API_BUSTOOLS_NO_OBJECT;
   }
   ...
   *******************************************************************/
   ...
   for ( iFIFO = 0; iFIFO < nFIFO[cardnum]; iFIFO++ )
   {
      ...
      fprintf(hfMemFile,
         ...
         "  card=%d, PtrIndex=%d, #Events=%d, Overflow=%X, Head=%d, Tail=%d, Mask=%X\n",
   ...
   *******************************************************************/
   switch ( nType )
   {
      ...
      case EVENT_BC_MESSAGE:     /* BC message transacted                 */
         ...
         subaddress = msg.subaddr;
         wordCount  = msg.wcount;
         vbtReadRelRAM32[cardnum](cardnum, (BT_U32BIT *)&int_status, msgAddr + BC_DATA_STATUS_OFFSET, 1);
      ...
      case EVENT_BM_TRIG:        /* BM trigger event (start/stop)         */
         ...
         if((int_status & BT1553_INT_RT_RT_FORMAT) && BM_INT_ON_RTRT_TX[cardnum])
         {
            ...
            {  //BM_MBUF data is wrapped.
               ...
               vbtReadBMRAM[cardnum](cardnum, (BT_U16BIT *)&bmmsg, brdAddr + BM_CMD2_OFFSET, 1);
