      ...
      case API_TTM_XCLK: //Use 1Mhz external clock
         ...
         if(board_is_v5_uca[cardnum])
         {
            if((_HW_FPGARev[cardnum] & 0xfff)<0x500)
               break; //No support for external clock in F/W less 5.00
...
NOMANGLE BT_INT CCONV BusTools_IRIG_SetTime(BT_UINT cardnum,   // (i) card number
                                            BT_U64BIT timedate,     // (i) timedate to set -1 use system time
                                                               ...
                                                               //     0x10 = DDD:HH:MM:SS format
{
   ...
   if(flag & 0x10)
   {
      ss =  (BT_UINT)( timedate  & 0x00000000000000ff);
      mm =  (BT_UINT)((timedate  & 0x000000000000ff00)>>8);
      hh =  (BT_UINT)((timedate  & 0x0000000000ff0000)>>16);
      ddd = (BT_UINT)((timedate  & 0x0000000fff000000)>>24); //
