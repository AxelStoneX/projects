   ...
   API_PLAYBACK playbackData)  // (io) Control and Status structures
{
   ...
   if(pbv6[cardnum].filterFlag == messageNumberFilter)
   {
      ...
      if(nsTimeTag)
      {
         ...
         bt.time64/=1000;
         pbv6[cardnum].startTimeDelay = bt.ttime.microseconds;
      }
      else
         pbv6[cardnum].startTimeDelay = pbv6[cardnum].BMRecord.time.microseconds;
   }
   else
   {
...
#ifdef _TIME_CYCLE
   if(pbv6[cardnum].lastTagTime > tagTime.microseconds)
   {
      pbv6[cardnum].timeOffset+=1000000; // 1 second in uSec
   }
#endif //_TIME_CYCLE

   pbv6[cardnum].lastTagTime = tagTime.microseconds;
   pbv6[cardnum].hwTagTime = (((tagTime.microseconds+startLatency+pbv6[cardnum].timeOffset)-pbv6[cardnum].startTimeDelay) << 1);
...
static XMIT_MESSAGE buildXmitMessagev6(BT_U16BIT status, int count, int message_sync, BT_UINT cardnum)
{
   ...
   if((pbv6[cardnum].BMRecord.int_status & BT1553_INT_CHANNEL) == 0)
   {
      message.channel = 0;
   }
   else
...
NOMANGLE BT_INT CCONV BusTools_Playback_Checkv6(char * bmdfile, BT_UINT mins)         // (i) card number (0 - based)
{
   ...
   fread(&bmdHeader,sizeof(BMDX_HEADER),1,rptr);  
   ...
   while(!feof(rptr))
   { 
      fread(&bm_mbuf,sizeof(API_BM_MBUF),1,rptr);
...
NOMANGLE BT_INT CCONV BusTools_Playback_Check(char * bmdfile, BT_UINT mins )         // (i) card number (0 - based)
{
   ...
   fileApp = strtok(bmdfile,".");
   cmpVal = strcmp(fileApp,"bmdx");
